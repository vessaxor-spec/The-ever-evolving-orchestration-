...
| Current validated test suite | 804 tests passed |
| Current full repository validation | CI #602: 804 tests, 510 tracked-file layout checks, 41 schemas, valid linked configuration, regulated-specialist evidence pass, provider-diverse end-to-end pass |
...
The accepted Task Intent & Action Authority research direction was then recorded on top of that provenance baseline. Exact-head Reference Implementation CI #602 passed **804 tests**, **510 tracked-file layout checks**, regulated specialist evidence validation, **41 parsed JSON Schemas**, valid linked configuration with zero issues, and the provider-diverse end-to-end example. This is the current full repository validation scale; it does not replace CI #514 as the accepted substantive runtime-control baseline, create empirical provider-backed documentation replay evidence, or make the new authority research normative.
...
- [`../specification/final-execution-provenance.md`](../specification/final-execution-provenance.md): optional read-only projection of revalidated active-route evidence into `FinalOutcome`
