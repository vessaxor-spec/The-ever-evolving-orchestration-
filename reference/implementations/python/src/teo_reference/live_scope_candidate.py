...
class LiveScopeExpansionPolicy:
    ...
    def validate(self) -> None:
        ...
        for key in ("primary", "fallback", "primary_verifier", "desired_fallback_verifier"):
...
def evaluate_live_scope_candidate(
    ...
    *,
    ...
    The evaluator reports readiness only. It cannot add a task type to active runtime
    scope, select a different model, accept previews, or execute a provider call.
    """
    ...
    desired_fallback_verifier = expected["desired_fallback_verifier"]
    redispatches = (model_redispatch, provider_redispatch)
    fallback_redispatch_ready = all(
        dispatch is not None
        and _route_matches(dispatch.selected_implementation, expected["fallback"])
        and _route_matches(dispatch.verification.implementation, desired_fallback_verifier)
    ...
    gates = (
        ...
        LiveScopeCandidateGate(
            "exact_fallback_route_matches",
            _route_matches(primary.fallback_implementation, expected["fallback"]),
            "Candidate must preserve the canonical routine fallback recorded by the initial dispatch.",
        ),
        LiveScopeCandidateGate(
        ...
        LiveScopeCandidateGate(
            "fallback_executor_adapter_supported",
            bool(
                primary.fallback_implementation
                and _executor_supported(
                    primary.fallback_implementation.provider_family,
                    primary.fallback_implementation.model,
                )
            ),
            "The guarded executor must support the canonical fallback model before activation.",
        ),
        LiveScopeCandidateGate(
        ...
        LiveScopeCandidateGate(
            "fallback_verifier_adapter_supported",
            _verifier_supported(
                desired_fallback_verifier["provider_family"],
                desired_fallback_verifier["model"],
            ),
            "The desired fresh provider-diverse fallback verifier already has a guarded verifier adapter.",
        ),
        LiveScopeCandidateGate(
            "fallback_redispatch_has_fresh_provider_diverse_verifier",
            fallback_redispatch_ready,
            "Both model- and provider-failure redispatch must resolve the declared fallback with a fresh provider-diverse verifier.",
        ),
        LiveScopeCandidateGate(
