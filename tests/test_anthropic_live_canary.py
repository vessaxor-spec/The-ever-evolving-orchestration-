...
def connection(headers: Mapping[str, str] | None = None) -> HeaderProviderConnection:
    return HeaderProviderConnection(
        provider_family="anthropic",
        authorization_headers=headers or {"x-api-key": "test-secret-key"},
    )
...
def transport_response(
    ...
    *,
    ...
    def transport(url: str, body: bytes, request_headers: Mapping[str, str], timeout: float):
...
def test_connection_layer_accepts_oauth_style_authorization_without_route_change(tmp_path: Path) -> None:
    calls: list[dict] = []
    oauth_connection = connection({"authorization": "Bearer delegated-oauth-token"})
    response = execute_anthropic_canary_once(
        dispatch(),
        oauth_connection,
        artifact_dir=tmp_path,
    ...
    assert response.status == "succeeded"
    assert calls[0]["headers"]["authorization"] == "Bearer delegated-oauth-token"
    assert calls[0]["body"]["model"] == "claude-haiku-4-5"
...
def test_connection_credentials_are_not_serialized_into_provider_request() -> None:
    ...
    assert "delegated-oauth-token" not in serialized
    assert "authorization" not in serialized.lower()
    assert "api_key" not in serialized.lower()


def test_live_canary_refuses_non_canary_task_before_network_call(tmp_path: Path) -> None:
    calls: list[dict] = []
...
def test_live_canary_refuses_consequential_risk_before_network_call(tmp_path: Path, risk: str) -> None:
    calls: list[dict] = []
    with pytest.raises(ProviderAdapterContractError, match="refuses high and critical"):
        execute_anthropic_canary_once(
            dispatch(risk_level=risk),
            connection(),
            artifact_dir=tmp_path,
            transport=transport_response(200, {}, calls=calls),
        )
...
def test_live_canary_refuses_wrong_provider_connection_before_network_call(tmp_path: Path) -> None:
    calls: list[dict] = []
...
def test_live_canary_refuses_non_haiku_model_before_network_call(tmp_path: Path) -> None:
    calls: list[dict] = []
    with pytest.raises(ProviderAdapterContractError, match="Haiku 4.5"):
        execute_anthropic_canary_once(
            dispatch(model="claude-sonnet-5"),
            connection(),
            artifact_dir=tmp_path,
            transport=transport_response(200, {}, calls=calls),
        )
...
def test_transport_failure_is_transient_and_single_attempt(tmp_path: Path) -> None:
    calls = 0

    def failing_transport(url: str, body: bytes, headers: Mapping[str, str], timeout: float):
        nonlocal calls
        calls += 1
...
def test_connection_cannot_override_protocol_headers() -> None:
    adapter = AnthropicMessagesAdapter(
        connection({"anthropic-version": "1999-01-01", "authorization": "Bearer token"}),
        transport=transport_response(200, {}),
    )
    request = ProviderExecutionRequest.from_dispatch(dispatch())
    with pytest.raises(ProviderAdapterContractError, match="API version"):
