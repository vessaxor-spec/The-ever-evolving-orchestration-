...
STAGING_PATH = REPO_ROOT / "docs" / "history" / "activation" / "platform-reliability-operations-staging.yaml"
...
EXPECTED_SPECIALISTS = {
    "performance-engineer": "performance_engineering",
    "finops-engineer": "finops_engineering",
    "site-reliability-engineer": "site_reliability",
...
def test_operations_specialists_and_workers_match_canonical_blobs() -> None:
...
def test_operations_specialist_depth_and_separation_are_preserved() -> None:
    staging = load_yaml(STAGING_PATH)
    texts = {
    ...
    assert "Does not replace QA" in texts["performance-engineer"]
    assert "Does not replace Finance" in texts["finops-engineer"]
    assert "Does not replace DevOps" in texts["site-reliability-engineer"]
    assert "Does not replace AI Engineering" in texts["mlops-engineer"]


def test_operations_activation_is_staged_and_boundaries_are_locked() -> None:
    staging = load_yaml(STAGING_PATH)
    ...
    assert "site_reliability_must_not_absorb_devops_or_incident_command" in rules
    assert "performance_engineering_must_not_absorb_qa" in rules
    assert "mlops_must_not_absorb_ai_engineering_data_engineering_or_applied_science" in rules
    assert "finops_must_not_substitute_for_finance_procurement_product_or_risk_authority" in rules
    assert "six_card_regulated_evidence_pilot_scope_remains_unchanged" in rules


def test_operations_wave_is_not_in_active_routing_or_registry() -> None:
