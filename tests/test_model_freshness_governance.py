...
AI_INSTRUCTIONS_PATH = REPO_ROOT / "AI_INSTRUCTIONS.md"
...
def test_ai_instructions_make_freshness_policy_mandatory() -> None:
    text = AI_INSTRUCTIONS_PATH.read_text(encoding="utf-8")
    assert "policy/governance/model-freshness.yaml" in text
    assert "Pretrained, cached, remembered, or previously documented model information is not authoritative" in text
    assert "A newer model does not automatically replace an existing route" in text
