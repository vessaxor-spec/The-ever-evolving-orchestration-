...
The latest Host Integration executable research validation is Reference Implementation CI #739: **964 automated tests**, **543 tracked-file layout checks**, regulated specialist evidence validation, **41 JSON Schema** parses, valid linked configuration, and the provider-diverse artifact-bound end-to-end reference lifecycle. That run validated the hardened non-normative Fresh-AI cross-session assurance model after empirical localhost trial 001. The real two-session no-reminder trial supports fresh-session standing-hook and **routing continuity**, but full end-to-end assimilation **FAILED** because local Qwen generated the artifact instead of the Gemini 3.5 Flash-Lite implementation selected by TEO and no live Claude Sonnet 5 verifier executed. The hardened validator now returns `routing_continuity_only` for research simulation and requires authenticated selected-versus-observed executor and verifier identity plus exact output/verification-record digest binding for a full PASS. Restart-persistent host hooks, production scheduler enforcement, tenant/account/credential binding, dynamic hook/plugin authority discovery, remote authenticity, durable distributed state, and compromised-host resistance remain explicitly unproved.
...
Since those validation rounds, eleven provider-independent adversarial slices plus an integrated assimilation/conformance replay have converted several previously open integration questions into executable evidence:
...
- integrated Fresh-AI assimilation/conformance composes the previously separate controls into one host path and makes "Assimilation is not installation" executable: plugin/product framing, parallel host routing, collapsed responsibility semantics, one-time hooks, activation without shadow evidence, one-demo-only continuity, same-task replay, staged-scope bypass, revoked admission, artifact mutation, human-gate loss, autonomy loss, recursive re-entry, and the original Host Integration premortem paths are rejected or replayed, while real cross-session persistence and production enforcement remain open;
- the Fresh-AI cross-session trial framework binds a frozen setup commitment to a later neutral challenge and distinct-session evidence packet; empirical localhost trial 001 supports fresh-session/no-reminder standing-hook and routing continuity but failed full end-to-end assimilation when Qwen replaced the TEO-selected Gemini executor and a research fixture stood in for the selected Claude verifier; the hardened validator now limits research simulation to `routing_continuity_only` and requires authenticated selected-versus-observed executor/verifier identity plus digest binding for full PASS.
...
Empirical localhost Fresh-AI trial 001 is preserved at [`research/runtime/2026-08-15-local-fresh-ai-cross-session-trial-001.md`](research/runtime/2026-08-15-local-fresh-ai-cross-session-trial-001.md). The trial proved that a distinct no-reminder fresh session inherited the standing TEO hook and entered TEO routing/authority before task execution, but independent falsification rejected end-to-end PASS because the observed Qwen executor and fixture verifier did not match the Gemini and Claude identities selected by TEO. PR #169 hardened that boundary, and Reference Implementation CI #739 passed **964 automated tests**, **543 tracked-file layout checks**, regulated specialist evidence validation, **41 JSON Schema** parses, valid linked configuration, and the provider-diverse artifact-bound reference lifecycle. This is routing-continuity evidence plus a negative end-to-end result, not a live selected-provider assimilation PASS.
...
The portfolio/task-admission authority-separation slice preserved two distinct negative findings before acceptance. Reference Implementation CI #696 completed with **913 passed and 1 failed** because the sibling-admission test expected a later task-ID mismatch even though the gate correctly failed earlier on admission-ID mismatch. Separately, Security and Authority Boundaries review found that the first candidate did not bind session revalidation to the exact host-issued TEO session identity. The correction stored and revalidated that identity, added a fabricated-session counterexample, and kept the TEO-facing gateway limited to exact admission claim and revalidation. Clean corrected Reference Implementation CI #703 passed **915 automated tests**, **535 tracked-file layout checks**, regulated specialist evidence validation, **41 JSON Schema** parses, valid linked configuration, and the provider-diverse artifact-bound end-to-end lifecycle. This supports conformant process-local separation only; remote/distributed host-admission authenticity, restart durability, production scheduler integration, tenant/account binding, compromised-host bypass, and distributed duplicate-work prevention remain open.

The integrated assimilation/conformance slice was built only after materializing and operating the exact merged repository in a sandbox. Initial clean CI #717 passed **940 tests** and **539 tracked-file layout checks**. Independent review then found that two receipts could still represent the same setup task; the harness was hardened to require two distinct post-assimilation task IDs and to keep cross-session continued-use proof explicitly unsupported. The 26-case targeted hardening suite passed before commit, and clean hardened CI #720 passed **941 automated tests**, **539 tracked-file layout checks**, regulated specialist evidence validation, **41 JSON Schema** parses, valid linked configuration, and the provider-diverse artifact-bound end-to-end lifecycle. The full fresh-AI process is preserved in [`research/roadmaps/host-integration-assimilation-protocol.md`](research/roadmaps/host-integration-assimilation-protocol.md), with dated evidence in [`research/runtime/host-integration-integrated-conformance-assimilation-2026-08-15.md`](research/runtime/host-integration-integrated-conformance-assimilation-2026-08-15.md). The stronger two-session question is now executable through [`research/roadmaps/host-integration-fresh-session-trial.md`](research/roadmaps/host-integration-fresh-session-trial.md); CI #735 passed **958 automated tests** and **542 tracked-file layout checks** for that framework. No empirical host PASS has yet been recorded, so process-local success and validator availability still do not prove a fresh session inherited TEO without a reminder or that an arbitrary host cannot bypass its own integration.

Before normative promotion, remaining evidence includes provider/model input economics, end-to-end latency and task adherence, production-grade external-adapter package provenance and authority-controlled loading, dependency/transitive-code identity, revocation/update and downgrade semantics, distributed host/TEO authority synchronization, production resource-target canonicalization and containment, credential/account/tenant scope binding, production-grade remote or distributed dispatch and exact-action authenticity/replay beyond the brokered conformant process-lifetime path, result/effect receipt authenticity, restart-durable and distributed retry-budget coordination, production compatibility-catalog provenance and governance, remote freshness authenticity, downgrade resistance, distributed/restart-durable freshness coordination and expiry semantics, production-grade portfolio/task-admission authenticity, durable revocation/admission state, scheduler integration, tenant/account binding, dynamic authority-surface discovery for executable hooks, plugins/loaders, and constructed paths, restart-durable and distributed recursion state, production scheduler recursion/recovery containment, and independent review against a parallel routing or authority plane.
